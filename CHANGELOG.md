...
- Auth Hub per-project task client with independent token policies, server-side operation and exact GitHub repository/scope allowlists, persistent idempotency keys, `project.*` own-task isolation, request-bound strict readiness projection, total-budget polling, no-redirect transport, HTTPS-443 remote target enforcement with IP-literal rejection, and packaged `ai-link-auth-hub` CLI.
- Project-director roadmap output that records the completed Auth Hub merge chain and points the next decisions to real read-only acceptance, low-noise notification activation, and remote deployment gates.

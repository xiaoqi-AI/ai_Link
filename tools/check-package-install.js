...
try {
  ...
  addCheck("dist Auth Hub project cli", existsSync(path.resolve(cwd, "dist", "authHub", "projectTask.js")) ? "pass" : "fail", "dist/authHub/projectTask.js", "files");
  ...
  const installedAuthHubCli = path.join(consumerDir, "node_modules", "@xiaoqi-ai", "ai-link", "dist", "authHub", "projectTask.js");
  ...
  addCheck("installed Auth Hub project cli", existsSync(installedAuthHubCli) ? "pass" : "fail", "node_modules/@xiaoqi-ai/ai-link/dist/authHub/projectTask.js", "install");
  ...
  if (existsSync(installedAuthHubCli)) {
    const helpResult = runNode([installedAuthHubCli, "--help"], consumerDir);
    addCheck(
      "installed Auth Hub project cli help",
      helpResult.status === 0 && helpResult.stdout.includes("Auth Hub 项目客户端") ? "pass" : "fail",
      helpResult.status === 0 ? "help rendered" : cleanFailure(helpResult),
      "cli"
    );
  } else {
    addCheck("installed Auth Hub project cli help", "fail", "installed Auth Hub project cli unavailable", "cli");
  }
} finally {

...
平台授权 P0.1 增加 `platform_auth_collect` 合同和本机私有连接器注入边界。小红书首批只允许会话检查、人工登录和只读搜索；公众号首批只增加官方 API 健康检查。受信任的私有模块必须位于 `runtime/private/`，通过 `AI_LINK_PRIVATE_CONNECTOR_MODULE` 仅注入本地执行器；公开结果只保留稳定状态、具体链接、数量和有限诊断。完整范围、决策和人工门禁见 `docs/10-product/platform-auth-connectors-p0.md` 与 `docs/20-architecture/connector-contracts.md`。

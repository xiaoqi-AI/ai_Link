...
项目总监当前基线、三模块进度、PR 合并链和人工决策以 `docs/10-product/ai-link-program-control.md` 为准。本文件继续保留阶段设计，不再作为“当前是否已实施”的唯一状态源。
...
2026-07-11 优先级已更新：首个真实平台方向调整为“小红书只读会话/搜索 + 公众号官方 API 健康检查/草稿”，先完成公开合同和私有加载机制，再分别进入真实账号门禁。朱雀AI维持现有预留，不抢占该 P0。详细边界见 `platform-auth-connectors-p0.md`。
...
目标：把小红书只读登录/搜索和公众号官方 API 健康检查、草稿能力放进私有边界内验证。

交付：

- 私有仓保存连接器代码，本机 `runtime/private/` 保存浏览器 Profile 和运行态。
- 小红书只实现会话检查、人工登录和只读搜索，真实搜索成功才算 `valid`。
- 公众号优先官方 API，先区分凭据、出口 IP 白名单、限流和服务不可用，再实现草稿。
- 公众号后台浏览器补位和朱雀AI真实连接器分别另立需求。

验收：

- 小红书可返回至少一条已验证的具体笔记链接，且不含会话参数。
- 公众号健康检查能准确识别 `official_api_ip_not_whitelisted`，白名单完成后才验收草稿。
- 任何正式发布仍必须经过控制台或邮件审批。

...
- `Provider Live Verification` 手动 GitHub workflow，使用 Bitwarden Secrets Manager 注入 provider key。
...
- 真实 provider API key 只允许存在于 Bitwarden Secrets Manager 或本机会话环境中；GitHub Actions 只保存 `BW_ACCESS_TOKEN` 和非敏感的 Bitwarden secret ID 变量。

...
- `ai-link runs list` 和 `ai-link runs show <id>` 可查看本地运行索引和单条记录。
- `workflow run --resume-from <id|latest>` 可从本地 workflow 记录续跑；`--from-stage <stage>` 可从指定阶段重跑。
...
- `runs list/show` 只读取 `runtime/tmp/ai-link-runs/` 下的记录；即使索引内容被手动改坏，也不读取目录外文件。
- `--resume-from` 只接受本地 workflow 记录，不读取目录外文件；记录已经完成全部阶段时，需要显式 `--from-stage` 才会重跑。
...
1. 在运行记录基础上评估 workflow 手动审批节点。
2. 为 Agent 长任务增加回调或轮询状态记录。
3. 评估是否需要 `runs prune` 清理本地旧记录。

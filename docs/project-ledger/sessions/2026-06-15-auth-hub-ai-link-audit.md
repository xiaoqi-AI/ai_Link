...
- 服务端同时追加 `ai_link.audit` 审计事件，可通过任务详情和 `GET /api/audit` 查询。
...
- 通用脱敏规则仍然保守；只有 `ai_link.audit` 事件和规范化后的 `task.result.aiLinkAudit` 会保留 token 计数字段。
...
1. 让真实 AI Link workflow handoff 或本地 executor 自动把 CLI `--record` audit 摘要回传到授权中枢。
2. 给控制台增加更易扫读的 provider/model/policy 表格。
3. 为 `GET /api/audit` 增加 event type 筛选。

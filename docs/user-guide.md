...
`skill draft` 会把自然语言说明转换成候选 `routes` 和 `workflows`；加 `--write .ai-link/local.yaml` 时默认只预览，只有再加 `--yes` 才会写入。`workflow run` 会按配置串联多个阶段。默认 `auto_ops` 示例会先运行 `auto_ops.research`，再把调研输出交给 `auto_ops.article_draft`。Codex skill 示例见 `examples/codex-skills/auto-ops-ai-link/SKILL.md`。
